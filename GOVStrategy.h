...
struct GOVStrategy {
    ...
    struct Hash : DoubleHashSequence<k + 1, Hashable> {
        Hash(const Hashable &str, uint32_t seed)
            : DoubleHashSequence<k + 1, Hashable>(str, seed){};
        uint32_t chunkHash() {
            return (*this)[k];
        };
    };
    ...
    inline static bool retrieve(const Hash &H, const uint8_t *ptr,
                                uint32_t logicalOffset, uint32_t logicalSize) {
        bool res = false;
        for (int i = 0; i < k; ++i) {
...
bool GOVStrategy<l, Hashable>::runConstruction() {
    ...
    vector<size_t> varsSorted;
    varsSorted.reserve(vars.size());
    {
        using p32 = pair<uint32_t, uint32_t>;
        vector<p32> degToVar;
        degToVar.reserve(vars.size());
    ...
    for (;;) {
        ...
        if (nextActivation == varsSorted.size())
            break;
        size_t vid = varsSorted[nextActivation++];
        Variable &v(vars[vid]);
        if (v.state == VAR_SOLVED)
            continue;
    ...
    if (!fourRussianGauss(M, rhss, activeVarSol))
        return false;
    sol.resize(nvars);
 ...
 * and should have zero right hand sides as well */
    ...
    for (int rr = r - 1; rr >= 0; --rr) {
        solution.setBit(pivots[rr],
                        scalarProduct(solution, M[rr], pivots[rr]) ^ rhss[rr]);
    }
