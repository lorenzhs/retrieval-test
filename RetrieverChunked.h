...
struct ChunkedStrategyExample {
    ...
    inline static bool retrieve(const Hash& H, const void* ptr,
...
struct RetrieverChunked {
    ...
    RetrieverChunked(Configuration config) : config(config) {}

    RetrieverChunked(const vector<Hashable>& keys, const vector<bool>& values,
    ...
    void Construct(const vector<Hashable>& keys, const vector<bool>& values) {
        ...
        vector<Strategy> solvers;
        solvers.reserve(nChunks);
        ...
        vector<uint32_t> logicalSizes;
        logicalSizes.reserve(nChunks);
