...
public:
    ...
    uint32_t stop() {
        uint32_t d =
            duration_cast<std::chrono::microseconds>(Clock::now() - currStart).count();
        times.push_back(d);
        return d;
    }
    uint32_t median_mus() {
...
inline uint32_t hash_pair_multiply_shift(uint64_t x, uint64_t a1, uint64_t a2,
                                         uint64_t b) {
...
inline unsigned extract(const void* array, int blockID) {
    ...
    return (*(uint32_t*)((uint8_t*)array + l * blockID / 8) >> (l * blockID % 8)) &
           ((1 << l) - 1);
}
template <>
inline unsigned extract<8>(const void* array, int blockID) {
    return ((uint8_t*)array)[blockID];
}
template <>
inline unsigned extract<16>(const void* array, int blockID) {
    return ((uint16_t*)array)[blockID];
}
template <>
inline unsigned extract<32>(const void* array, int blockID) {
...
inline bool parityl(uint32_t x) {
    return __builtin_parityl(x);
}
template <>
inline bool scalarProduct<unsigned long>(unsigned long i1, unsigned long i2) {
    return __builtin_parityl(i1 & i2);
}
template <>
inline bool scalarProduct<unsigned char>(unsigned char i1, unsigned char i2) {
    return __builtin_parity(i1 & i2);
}
template <>
inline bool scalarProduct<unsigned long long>(unsigned long long i1,
                                              unsigned long long i2) {
    return __builtin_parityll(i1 & i2);
}
