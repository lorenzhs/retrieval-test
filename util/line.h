...
struct Line {
    ...
    struct BitsView {
        ...
        int shift;
        uint64_t mask;
        BitsView(int firstBit, int numBits)
            : byteOffset(firstBit >> 3), shift(firstBit & 7),
              mask((1 << numBits) - 1) {}
    };
    ...
    struct BitView {
        ...
        uint8_t mask;
        BitView(int offset)
            : byteOffset(offset >> 3), mask(1 << (offset & 7)) {}
    };

    inline uint64_t get(const BitsView &view) const {
        return ((*((uint64_t *)(((uint8_t *)v.data()) + view.byteOffset))) >>
                view.shift) &
               view.mask;
    }
