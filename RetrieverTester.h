...
struct RetrieverTester {
    ...
    static bool toRetrieve(const string& s) {
        return s.length() & 1;
    }
    static bool toRetrieve(const UniqueRandomInt64& s) {
        return s.val & 1;
    }

    bool verify(const Retriever& R, const vector<Hashable>& keys,
        ...
        for (int i = 0; i < keys.size(); ++i)
            if (R.retrieve(keys[i]) != values[i])
                errors++;
        errors ? (cout << errors << " errors found!") : cout << "Done",
            cout << endl;
        return errors != 0;
    }

    void testQueries(const Retriever& R, const vector<Hashable>& keys,
                     PerformanceLog& log, int times = 1) {
        uint32_t buff = 0;
        Timer hashTimer;
    ...
    void testAll(const vector<Hashable>& keys,
                 typename Retriever::Configuration config, int times = 1,
                 string texConfig = "") {
        stringstream ss;
#ifdef __GNUC__
        int status;
        char* realname =
            abi::__cxa_demangle(typeid(Retriever).name(), 0, 0, &status);
        ss << realname << config;
        free(realname);
#else
        ...
        string name = ss.str();
        prettifyName(name);
        if (!PerformanceLog::logs.count(name))
            PerformanceLog::creationOrder.push_back(name);
        ...
        cout << "Construction Time ([mus/key] / [mus]) " << timePerKeyMS
             << " / " << dc << endl;
